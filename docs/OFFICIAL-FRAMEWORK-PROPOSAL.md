...
- **The bootstrap `packages/tests-billing-servicing/` is created by manually expanding the templates with `slug=billing-servicing`**, not by hand-writing files that look approximately like the templates. To prove parity *now* (before the scaffold script exists), Phase 0 ships a tiny verification script `packages/tooling/scripts/verify-bootstrap-vs-templates.sh` that diffs the manually-expanded bootstrap against the template tree (with placeholders substituted in-process) and fails CI if any byte differs. When the real scaffold script lands in Phase 1, the same verification re-runs against the script's output. **This eliminates D-34's drift problem on day one.**
- The walking-skeleton spec lives at `packages/tests-billing-servicing/tests/smoke/login.spec.ts` and is itself a template artifact: `packages/tooling/templates/team/tests/smoke/login.spec.ts.tpl`. Future scaffolded teams get the same spec retagged with their own area tag. **Naming-collision policy:** when the scaffold script runs against an existing slug (including `billing-servicing`), it refuses to overwrite without `--force`. The walking-skeleton spec is therefore protected; teams that want to add their own first smoke spec name it differently (e.g. `home.spec.ts`, `dashboard.spec.ts`), as documented in `docs/SCAFFOLD.md`.
...
1. **API client first.** Build `packages/framework/src/api/client.ts` (auth, retry, env-aware base URL, the production safety guard from D-09) and the typed `/qa/*` wrappers used by `C25193`'s isolation model: `DummyFirmApi`, `InvitationApi`. Each ships with its Zod schema and a unit test that asserts schema parsing against a recorded fixture from a real qa2 response.
2. **Factories.** Build `FirmFactory` and `ProspectFactory` on top of the API client.
3. **`firm.fixture` and `worker-firm.fixture`.** Lift `packages/legacy-poc/tests/_helpers/worker-firm.js` (~300 LOC, the program's most valuable asset) into `packages/framework/src/fixtures/workerFirm.fixture.ts` as a typed worker-scoped fixture. This is the lift that enables every future write-path test in every team package; it is not optional and must not be deferred to Phase 4.
...
2. Build any area-specific Page Objects under `packages/tests-billing-servicing/src/pages/<area>/`. Promote any Page Object that proves reusable to `packages/framework/src/pages/` per the promotion rule (Section 4.2.2).
...
- **Sunset the legacy POC.** Delete the entire `packages/legacy-poc/` package: removes its `tests/`, `tests/_helpers/`, the CommonJS shim, the legacy JS reporter, the legacy `playwright.config.js`, and the legacy `scripts/`. Remove the `legacy-poc` entry from the workspace `package.json`. Drop `allowJs` from `tsconfig.base.json`. The single-PR cutover from JS-on-Run-175 to TS-on-Run-175 (the framework's TS reporter, decision D-15) lands in the same change.
