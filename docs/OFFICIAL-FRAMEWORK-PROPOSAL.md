...
> - Node 24 installed (matches `.nvmrc`).
...
|---|---|---|
| Node.js | `24.x` (declared in `package.json` `engines` and `.nvmrc`) | Current release line; required by Playwright 1.59+. |
...
**Deliverables.**
- Workspace root: `package.json` with `"workspaces": ["packages/*"]` and `engines.node = ">=24"`, `package-lock.json`, `tsconfig.base.json` (with framework path aliases), `.env.example`, `.nvmrc`, `CODEOWNERS`, `.eslintrc.legacy-areas.json`.
...
- [ ] ~~Branch protection enforces lint + type check + PR gate per package.~~ **DEFERRED (2026-04-09)** — GitHub branch protection (and rulesets) for private repos requires GitHub Pro or a paid org plan; the current repo is on the Free plan and is private. Workflows still run on every PR (PR-gate, nightly, scaffold-test) and surface red/green status — they just do not gate merges. Re-enable trigger: whichever lands first of (a) M3 second QA contributor (Section 6.12), at which point required reviews become meaningful and the cost calculus for GitHub Pro changes; (b) the repo moves to a GeoWealth GitHub organization with a Team plan; (c) the repo is made public. Until then this exit criterion is intentionally waived for solo development. The risk is bounded: the only loss is mechanical protection against `git push --force master` and accidental branch deletion — both rare in solo work, both reversible from the local clone.
...
| CI platform provisioned and accessible from QA repo | Platform / DevOps | Phase 1 | D-02 = GitHub Actions; workflows landed in `6765f91`. Secrets provisioned 2026-04-09. Branch protection deferred — see Section 6.3 Phase 1 exit criterion for trigger conditions |
