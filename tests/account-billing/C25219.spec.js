// @ts-check
/**
 * TestRail C25219 — Household: Adjustment/Expiration Date - Percent [%] -
 *                   Admin and Non-Admin
 *
 * Source: https://testrail.geowealth.com/index.php?/cases/view/25219 (Run 206)
 * Refs:   GEO-11480
 *
 * Mirror of C25245 but on the household-level URL (clientTypeCd=5). We
 * use workerFirm.household so each worker gets an isolated household —
 * no contention with other tests touching firm-106 adjustments.
 *
 * Phase 1 (admin / workerFirm admin):
 *   - open Edit Household Billing Settings on workerFirm.household
 *   - click "Add An Adjustment" in the Advisor bucket if needed
 *   - set Adjustment Type = "Percent [%]", value, expiration date
...
test('@pepi C25219 Household Adjustment/Expiration Date - Percent', async ({
  ...
  await test.step('Phase 1: set Advisor adjustment to Percent + value + expiration', async () => {
    ...
    const currentAmount = await page
      ...
      .inputValue()
      .catch(() => '');
    const percentValue = currentAmount.replace(/[^\d]/g, '') === PERCENT_A ? PERCENT_B : PERCENT_A;
    ...
    console.log(
      `[C25219] advisor adjustment: percent ${JSON.stringify(currentAmount)} -> ${percentValue}, date ${JSON.stringify(currentDateBefore)} -> ${dateValue}`
    );
  ...
  await test.step('Phase 1.2: History shows 3 adjustment rows for Advisor billing', async () => {
    ...
    // eslint-disable-next-line no-console
    console.log(
      `[C25219] history rows (${rowRecords.length}), unique settings: ${JSON.stringify(uniqueSettings)}`
    );
