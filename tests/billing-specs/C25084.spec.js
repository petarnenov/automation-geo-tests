...
test('@pepi C25084 Billing Spec Grid Shows Account Min/Max Columns', async ({
  page,
}) => {
  ...
  await test.step('Reset grid to "System View" so column visibility starts from defaults', async () => {
    ...
    await page
      .locator('a[data-type="listOption"][data-value="System View"]')
      .click();
  ...
  await test.step('Account Min/Max columns are NOT in the default visible columns', async () => {
    ...
    await expect(
      page.getByRole('columnheader', { name: 'Account Min' })
    ).toHaveCount(0);
    await expect(
      page.getByRole('columnheader', { name: 'Account Max' })
    ).toHaveCount(0);
  });
  ...
  await test.step('Enable the Account Min and Account Max column toggles', async () => {
    ...
    await expect(
      overlay.locator('input#applyMinFeesOnAccountLevelFlagField')
    ).toBeChecked();
    await expect(
      overlay.locator('input#applyMaxFeesOnAccountLevelFlagField')
    ).toBeChecked();
    // The grid only re-renders new columns AFTER "Confirm & Reload" is clicked.
    await overlay
      .getByRole('button', { name: 'Confirm & Reload' })
      .click();
  });
  ...
  await test.step('Account Min and Account Max columns appear in the grid header', async () => {
    ...
    await expect(
      page.getByRole('columnheader', { name: 'Account Max' }).first()
    ).toBeVisible({ timeout: 10_000 });
  });
  ...
  await test.step('Each row has a Y or N value in both Account Min and Account Max columns', async () => {
    ...
    const accountMinHeader = page
      .getByRole('columnheader', { name: 'Account Min' })
      .first();
