...
test('@pepi C25041 Correct Advisor Target Types Displayed by Billing Type', async ({ page }) => {
  ...
  /**
   * Find first master row in the BillingRuns grid (skip OngoingBillingRunsGrid
   * by requiring `[col-id="billingRunStatuses"]` on the grid container) where:
   *   Status="Completed", Published="N", target text starts with "Advisor(s):".
   */
  const findAdvisorRow = async () => {
  ...
  const handle = await findAdvisorRow();
  const el = handle.asElement();
  expect(el, 'advisor-target Completed Unpublished row should exist').toBeTruthy();
